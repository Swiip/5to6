 ...
 */
var breweryDbHost = 'http://api.brewerydb.com/';

var proxyBrewery = httpProxy.createProxyServer({
  target: breweryDbHost
});
...
proxyBrewery.on('error', errorHandler);
...
function proxyMiddleware(req, res, next) {
   ...
   */
  if (/^\/v2/.test(req.url)) {
    req.headers.host = 'api.brewerydb.com';
    proxyBrewery.web(req, res);
  } else {
    next();
  }
}
